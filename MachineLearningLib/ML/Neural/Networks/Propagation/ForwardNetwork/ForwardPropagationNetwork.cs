...
{
    [Serializable]
    public class ForwardPropagationNetwork : IPropagationNetwork
    {
        ...
        {
            info.AddValue("layerCount",     _layerCount,        typeof(int[]));
            info.AddValue("layerFeedCount", _layerFeedCount,    typeof(int[]));
            info.AddValue("weights",        _weights,           typeof(double[]));
            info.AddValue("neuronValues",   _neuronValues,      typeof(double[]));
            info.AddValue("weightsOffset",  _weightsOffset,     typeof(int[]));
            info.AddValue("layerOffset",    _layerOffset,       typeof(int[]));


            string[] typeNames = new string[_activationFunction.Length];

            for(int i = 0; i < _activationFunction.Length; ++i)
            {
                typeNames[i] = _activationFunction[i].FullTypeName;
            }
            

            info.AddValue("activationTypeNames", typeNames, typeof(string[]));
        }

        public ForwardPropagationNetwork(SerializationInfo info, StreamingContext context)
        {
            _layerCount     = (int[])info.GetValue("layerCount", typeof(int[]));
            _layerFeedCount = (int[])info.GetValue("layerFeedCount", typeof(int[]));
            _weights        = (double[])info.GetValue("weights", typeof(double[]));
            _neuronValues   = (double[])info.GetValue("neuronValues", typeof(double[]));
            _weightsOffset  = (int[])info.GetValue("weightsOffset", typeof(int[]));
            _layerOffset    = (int[])info.GetValue("layerOffset", typeof(int[]));


            string[] typeNames = (string[])info.GetValue("activationTypeNames", typeof(string[]));

            _activationFunction = new IActivationFunction[typeNames.Length];

            for(int i = 0; i < typeNames.Length; ++i)
            {
                Type actType = Type.GetType(typeNames[i]);
                _activationFunction[i] = (IActivationFunction)Activator.CreateInstance(actType);
            }
        }
